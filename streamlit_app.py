...
st.markdown("""
# 📈 Sipre Pro — Predictive Trading Signal Dashboard  
**Professional-grade predictive tool for stock and crypto markets using Prophet and LSTM models.**
""")

# Sidebar inputs for better UI separation
with st.sidebar:
    ...
    symbol_input = st.text_input("Enter symbol (e.g. LNR.TO or AAPL):", value="AAPL").upper().strip()

    filtered_symbols = [s for s in symbols_list if symbol_input in s] if symbol_input else symbols_list
    selected_symbol = st.selectbox("Or select from suggestions:", filtered_symbols) if filtered_symbols else None

    symbol = symbol_input if symbol_input else selected_symbol

    timeframe = st.selectbox("Select timeframe for historical data:", ["1mo", "3mo", "6mo", "1y", "2y", "5y"], index=3,
                             help="Length of past data to use for predictions")

    alert_email = st.text_input("Email for alerts (optional):")

    prophet_days = st.number_input("Number of days to predict (Prophet):", min_value=5, max_value=90, value=15, step=5)

    lstm_days = st.number_input("Number of days to predict (LSTM):", min_value=5, max_value=90, value=30, step=5)

    predict_button = st.button("Get Prediction & Signal")
...
def fetch_news_sentiment(symbol):
    try:
        # Placeholder: Ideally integrate an API for real sentiment
...
if predict_button:
    ...
    if df.empty or 'Close' not in df.columns:
        st.error("Failed to retrieve valid data for this symbol.")
        st.stop()
    ...
    # Fix for ambiguous Series comparisons: ensure scalars, not Series
    ...
    # Flatten series if 2D
    close_series = df_reset[close_col]
    if isinstance(close_series.iloc[0], (np.ndarray, list, pd.Series)):
        close_series = close_series.apply(lambda x: x[0] if hasattr(x, '__getitem__') else x)

    # Clip close prices for log transform
    ...
    prophet_df = pd.DataFrame({
        ...
        'y': np.log(prices_clipped.values.flatten())  # ensure 1D array for Prophet
    }).dropna()
    ...
    if prophet_df.shape[0] < 30:
        ...
        try:
            ...
            # Exponentiate with clipping for stability
            min_positive = 1e-3
            ...
            st.dataframe(forecast[['ds', 'yhat_exp', 'yhat_lower_exp', 'yhat_upper_exp']].tail(prophet_days),
                         use_container_width=True)
            st.download_button("📥 Download Prophet Forecast CSV", forecast.to_csv(index=False),
                               file_name=f"{symbol}_prophet_forecast.csv")
